...
p = polyfit(EArr,VmeanTuned,1); % ax + b
px = 0:50:3500;
py = polyval(p,px);

pMax = polyfit(EArr,VrangeTuned(1,:),1); % ax + b
pxMax = 0:50:3500;
pyMax = polyval(pMax,pxMax);

pMin = polyfit(EArr,VrangeTuned(2,:),1); % ax + b
pxMin = 0:50:3500;
pyMin = polyval(pMin,pxMin);
...
xlabel('Beam Energy (eV)')
ylabel('Deflector Plate Required Voltage (V)')
legend('Empirical Data','Linear Extrapolation')